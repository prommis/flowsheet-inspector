...
from idaes_fi.structfs.fsrunner import FlowsheetRunner, Context  # +++ ADDED
...
@FS.step("build")  # +++ ADDED
def build_model(context: Context):  # +++ changed: (m) -> (context: Context)
    m = ConcreteModel()  # +++ moved here from __main__
    ...
    context.model = m  # +++ changed: removed `return m`, assigned to context.model


@FS.step("set_solver")  # +++ ADDED: new step
def set_solver(context: Context):  # +++ ADDED
    solver = get_solver()  # IPOPT
    optarg = {"tol": 1e-6, "max_iter": 500}
    solver.options = optarg
    context.solver = solver  # +++ ADDED


@FS.step("set_operating_conditions")  # +++ ADDED
def set_inputs(context: Context):  # +++ changed: (m) -> (context: Context)
    m = context.model  # +++ ADDED
...
@FS.step("set_scaling")  # +++ ADDED
def scale_flowsheet(context: Context):  # +++ changed: (m) -> (context: Context)
    m = context.model  # +++ ADDED
...
@FS.step("initialize")  # +++ ADDED
def initialize_flowsheet(context: Context):  # +++ changed: (m) -> (context: Context)
    m = context.model  # +++ ADDED
...
@FS.step("add_costing")  # +++ ADDED
def add_costing(context: Context):  # +++ changed: (m) -> (context: Context)
    m = context.model  # +++ ADDED
...
@FS.step("solve_initial")  # +++ ADDED: extracted from main()
def solve_initial(context: Context):
    m = context.model  # +++ ADDED
    ...
    results = context.solver.solve(
        m, tee=context["tee"]
    )  # +++ changed: context.solver, tee=context["tee"]
    assert results.solver.termination_condition == TerminationCondition.optimal
    context.results = results  # +++ ADDED


@FS.step("initialize_costing")  # +++ ADDED: extracted from main()
def initialize_costing(context: Context):
    m = context.model  # +++ ADDED
    print()
    results2 = context.solver.solve(
        m, tee=context["tee"]
    )  # +++ changed: context.solver, tee=context["tee"]
    assert results2.solver.termination_condition == TerminationCondition.optimal
    context.results = results2  # +++ ADDED
    print("Initial solution process results:")
    report(m)  # display initial solution results


@FS.step("setup_optimization")  # +++ ADDED: extracted from main()
def setup_optimization(context: Context):
    m = context.model  # +++ ADDED
...
@FS.step("solve_optimization")  # +++ ADDED: extracted from main()
def solve_optimization(context: Context):
    m = context.model  # +++ ADDED
    print()
    print("Solving optimization problem...")
    opt_res = context.solver.solve(
        m, tee=context["tee"]
    )  # +++ changed: context.solver, tee=context["tee"]
    assert opt_res.solver.termination_condition == TerminationCondition.optimal
    context.results = opt_res  # +++ ADDED
    print("Optimal solution process results:")
    report(m)


if __name__ == "__main__":
    FS.run_steps()  # +++ changed: removed `m = ConcreteModel(); m = main(m)`
